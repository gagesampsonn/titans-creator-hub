(() => {
  ...
  async function copy(text, field) {
    try {
      ...
      field?.focus?.();
      field?.select?.();
      status.textContent = "Copy wasn't available. Select the text and copy it manually.";
    }
  }
  ...
  for (const button of document.querySelectorAll("[data-copy-link]")) {
    button.addEventListener("click", () => {
      ...
      if (links.has(product)) return copy(links.get(product), document.querySelector(`[data-link="${product}"]`));
    });
  }
  for (const button of document.querySelectorAll("[data-share-link]")) {
    button.addEventListener("click", async () => {
      ...
      if (!url) return;
      if (!navigator.share) { await copy(url, document.querySelector(`[data-link="${product}"]`)); return; }
      try {
        ...
        if (failure.name !== "AbortError") status.textContent = "Sharing wasn't available. Use Copy link instead.";
      }
    });
  }
  for (const button of document.querySelectorAll("[data-copy-hook]")) {
    button.addEventListener("click", () => {
      const hook = document.querySelector(`[data-hook="${button.dataset.copyHook}"]`);
      return copy(hook.textContent, hook);
    });
  }
