/**
 * December 24-28 Data Import Script
 * 
 * ADDS to existing data (does NOT replace Dec 1-23 data)
 * 
 * This script:
 * 1. Reads the Dec 24-28 export file
...
const EXCEL_FILE = 'C:\\Users\\gages\\Downloads\\CustomReport_Creator_Product_Shop 2025-12-24_2025-12-28.xlsx';
const LINKED_CREATORS_FILE = 'C:\\Users\\gages\\titans-creator-hub\\data\\linked_creators.csv';

// NEW date range for this import (Dec 24-28)
const DATE_START = '2025-12-24';
const DATE_END = '2025-12-28';

// For period summary, we'll combine with existing data
// Previous period was Nov 30 - Dec 22, new combined will be Nov 30 - Dec 28
const COMBINED_DATE_START = '2025-11-30';
const COMBINED_DATE_END = '2025-12-28';
...
async function importDec24to28Data() {
  console.log('📊 December 24-28 Data Import (ADDING to existing data)\n');
  ...
  if (rows.length > 0) {
    ...
    Object.keys(rows[0] as object).forEach(col => console.log(`   - ${col}`));
    console.log('');
  }
  ...
  console.log('📥 Importing Dec 24-28 product data...\n');
  ...
  for (const row of rows as Record<string, any>[]) {
    ...
    try {
      // Insert with NEW date range (Dec 24-28) - won't conflict with existing Dec 1-23
      const { error } = await admin.rpc('upsert_creator_product_metrics', {
        ...
        p_orders: orders,
        p_import_source: `${filename} (Dec 24-28 add)`
      });
  ...
  for (const [handle, newStats] of Object.entries(handleStats)) {
    ...
    // For comparison, use the existing prev values (they represent Nov 7-29)
    ...
    // Combine category breakdowns for top niche
    const combinedCategories: Record<string, number> = {};
    
    // Add existing categories (if we had them stored - we'll use new data for now)
    for (const [cat, gmvVal] of Object.entries(newStats.categoryBreakdown)) {
      combinedCategories[cat] = (combinedCategories[cat] || 0) + gmvVal;
    }
    
    // Determine top niche
    const categoryEntries = Object.entries(combinedCategories);
    let topNiche = existing?.top_niche || 'Uncategorized';
    let topNicheGmv = existing?.top_niche_gmv || 0;
    if (categoryEntries.length > 0) {
      categoryEntries.sort((a, b) => b[1] - a[1]);
      // Only update if new data has a stronger niche
      if (categoryEntries[0][1] > topNicheGmv) {
        topNiche = categoryEntries[0][0];
        topNicheGmv = (existing?.top_niche_gmv || 0) + categoryEntries[0][1];
      }
    }
    ...
    const { error } = await admin
      ...
      .insert({
        ...
        import_source: `Combined Nov30-Dec28 (added ${filename})`
      });
  ...
  console.log('═══════════════════════════════════════════════════════════\n');
  console.log(`✅ Imported: ${imported} new product records (Dec 24-28)`);
  ...
  console.log(`📊 New Sales Added (Dec 24-28 only):`);
  console.log('───────────────────────────────────────────────────────────');
  ...
  console.log('\n✨ December 24-28 data added successfully!\n');
}

importDec24to28Data().catch(console.error);
