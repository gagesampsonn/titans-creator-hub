/**
 * January 7-15 Data Import Script
 * 
 * ADDS to existing data (does NOT replace Dec 1 - Jan 6 data)
 * 
 * This script:
 * 1. Reads the Jan 7-15 export file
...
const EXCEL_FILE = 'C:\\Users\\gages\\Downloads\\CustomReport_Creator_Product_Shop 2026-01-07_2026-01-15.xlsx';
const LINKED_CREATORS_FILE = 'C:\\Users\\gages\\titans-creator-hub\\data\\linked_creators.csv';

// NEW date range for this import (Jan 7-15)
const DATE_START = '2026-01-07';
const DATE_END = '2026-01-15';

// For period summary, we'll combine with existing data
// Previous period was Dec 1 - Jan 6, new combined will be Dec 1 - Jan 15
const COMBINED_DATE_START = '2025-12-01';
const COMBINED_DATE_END = '2026-01-15';
...
async function importJan07to15Data() {
  console.log('📊 January 7-15 Data Import (ADDING to existing data)\n');
  ...
  console.log('📥 Importing Jan 7-15 product data...\n');
  ...
  for (const row of rows as Record<string, any>[]) {
    ...
    try {
      // Insert with NEW date range (Jan 7-15) - won't conflict with existing Dec 1 - Jan 6
      const { error } = await admin.rpc('upsert_creator_product_metrics', {
        ...
        p_orders: orders,
        p_import_source: `${filename} (Jan 7-15 add)`
      });
  ...
  for (const [handle, newStats] of Object.entries(handleStats)) {
    ...
    const { error } = await admin
      ...
      .insert({
        ...
        import_source: `Combined Dec1-Jan15 (added ${filename})`
      });
  ...
  console.log(`✅ Updated ${summariesUpdated} creator summaries\n`);

  // Print summary
  console.log('═══════════════════════════════════════════════════════════');
  console.log('                    IMPORT SUMMARY                          ');
  console.log('═══════════════════════════════════════════════════════════\n');
  console.log(`✅ Imported: ${imported} new product records (Jan 7-15)`);
  ...
  console.log(`📊 New Sales Added (Jan 7-15 only):`);
  console.log('───────────────────────────────────────────────────────────');
  ...
  // Calculate totals for new data
  const newGMV = sortedCreators.reduce((sum, [, stats]) => sum + stats.gmv, 0);
  const newItems = sortedCreators.reduce((sum, [, stats]) => sum + stats.items, 0);
  const existingGMV = sortedCreators.reduce((sum, [handle]) => sum + (existingByHandle[handle]?.total_gmv || 0), 0);
  const combinedTotalGMV = existingGMV + newGMV;
  
  console.log('\n───────────────────────────────────────────────────────────');
  console.log(`📈 New GMV (Jan 7-15): $${newGMV.toLocaleString('en-US', { minimumFractionDigits: 2 })}`);
  console.log(`📦 New Items (Jan 7-15): ${newItems.toLocaleString('en-US')}`);
  console.log(`💰 Combined Total GMV (Dec 1-Jan 15): $${combinedTotalGMV.toLocaleString('en-US', { minimumFractionDigits: 2 })}`);
  console.log('───────────────────────────────────────────────────────────');
  ...
  console.log('\n✨ January 7-15 data added successfully!\n');
}

importJan07to15Data().catch(console.error);
