...
async function render(mode = "ai", options = {}) {
  ...
  const element = selector => {
    if (!elements.has(selector)) elements.set(selector, { hidden: true, value: "", dataset: {}, events: {},
  ...
  vm.runInNewContext(readFileSync(new URL("../assets/affiliate-center.js", import.meta.url), "utf8"), {
    document: { querySelector: element, querySelectorAll: selector => selector === "[data-copy-link]" ? copyButtons : selector === "[data-share-link]" ? shareButtons : [], addEventListener() {} },
  ...
  return { element, copies, shares, copyButtons, shareButtons };
}
